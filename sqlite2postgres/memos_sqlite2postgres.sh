...
then
  ...
  sqlite3 "$litedb" ".mode insert memo" "SELECT id,uid,creator_id,created_ts,updated_ts,row_status,content,visibility,payload,pinned || '::boolean' FROM memo;" >> memos_2post.sql
  sed -i -e 's/char(/chr(/g' memos_2post.sql
fi
